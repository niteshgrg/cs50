...
int main(int argc, char* argv[])
{
    ...
    biold.biSizeImage = bi.biSizeImage;
    bi.biSizeImage = (abs(bi.biWidth) * 3 + paddingnew) * abs(bi.biHeight);
    bfold.bfSize = bf.bfSize;
    ...
    for (int i = 0, biHeight = abs(biold.biHeight); i < biHeight; i++)
    {
        for (int l = 0; l < n; l++)
        {
            fseek(inptr, bf.bfOffBits + i*((biold.biWidth*3)+paddingold), SEEK_SET);
