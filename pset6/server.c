...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // validating and seprating method
            haystack = line;
            ...
            // validating and seprating request_target
            needle = strchr(haystack, ' ');
            if(needle == NULL)
            {
                error(400);
            ...
            //validating and extracting http version
            ...
            strncpy(http_version, haystack, needle - haystack);
            http_version[needle - haystack] = '\0'; 
            ...
            // TODO: extract query and absolute-path from request_target
            needle = strchr(haystack, '?');
            ...
            {
                size=haystack+len-needle+1;
                size_abs=needle-haystack;
            }
            

            char absolute_path[size_abs+1];
            char query[size];
            strncpy(absolute_path, haystack, size_abs);
            absolute_path[size_abs] = '\0';
            //printf("%s\n", absolute_path);
            if(size == 1)
            {
                query[0] = '\0';
            ...
            // TODO: concatenate root and absolute_path
        
            char path[strlen(root)];
            strcpy(path, root);
            ...
            haystack = absolute_path;
            needle = strchr(haystack, '.');
            len=strlen(haystack);
            char extension[haystack+len-needle-1];
